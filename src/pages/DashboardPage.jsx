  ...
  return (
    <aside className="w-64 flex flex-col h-screen border-r border-white/5 bg-[#0d0b1a] sticky top-0">
      ...
      <div className="p-8 flex items-center gap-3">
        ...
        <span className="text-white text-2xl font-bold tracking-tight" style={{ fontFamily: "'Syne', sans-serif" }}>
          Fleekr
        </span>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-2 mt-4">
        {menuItems.map((item) => (
          ...
          >
            <Icon icon={item.icon} width={22} className={item.active ? "text-purple-400" : "group-hover:scale-110 transition-transform"} />
            <span className="font-medium text-[15px]">{item.label}</span>
            {item.active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-purple-400 shadow-[0_0_8px_#a855f7]" />}
          </button>
        ))}
      </nav>
  ...
  <div className="bg-white/5 border border-white/5 rounded-[28px] p-6 backdrop-blur-md hover:border-white/10 transition-colors">
    <div className="flex justify-between items-start mb-6">
      ...
      <div className={`flex items-center gap-1 text-[13px] font-bold px-2.5 py-1 rounded-full ${
        ...
        <Icon icon={isPositive ? "mdi:trending-up" : "mdi:trending-down"} width={16} />
        {trend}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#080710] flex font-sans selection:bg-purple-500/30">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        ...
        <header className="flex justify-between items-start mb-10">
          <div>
            <h1 className="text-white text-4xl font-bold mb-2 tracking-tight" style={{ fontFamily: "'Syne', sans-serif" }}>
              Creator Dashboard
            </h1>
            <p className="text-slate-400 font-medium">Track your performance and grow your audience ✨</p>
          </div>
      ...
      <button
        ...
        aria-label="Help"
      >
        <Icon icon="mdi:help" width={24} className="group-hover:rotate-12 transition-transform" />
      </button>
    </div>
  );
}
